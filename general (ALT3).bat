...
--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt" --hostlist="%LISTS%list-general-user.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --lua-desync=send:repeats=2 --lua-desync=syndata:blob=tls7 --new ^
...
--filter-tcp=80,443,%GameFilterTCP% --ipset="%LISTS%ipset-all.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --lua-desync=send:repeats=2 --lua-desync=syndata:blob=tls7 --new ^
--filter-tcp=80,443 --hostlist="%LISTS%list-google.txt" --hostlist-domains=googlevideo.com --lua-desync=send:repeats=2 --lua-desync=syndata:blob=tls7 --new ^
...
--filter-tcp=2053,2083,2087,2096,8443 --hostlist-domains=discord.media --lua-desync=send:repeats=2 --lua-desync=syndata:blob=tls7 --new ^
