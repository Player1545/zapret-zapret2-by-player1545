...
--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --lua-desync=hostfakesplit:repeats=4:tcp_ts_up:tcp_md5:host=ozon.ru --new ^
...
--filter-tcp=2053,2083,2087,2096,8443 --hostlist-domains=discord.media --lua-desync=hostfakesplit:repeats=4:tcp_ts_up:host=ozon.ru --new ^
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --lua-desync=hostfakesplit:repeats=4:tcp_ts_up:ip_id=zero:host=www.google.com